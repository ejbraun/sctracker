...
public class MvpReportService {
     ...
     *
     * <p>{@code request.roles()} holds a role name for a role-based run, or a character
     * {@code raw_name} for a role-less one ({@code map_configs.role_model = NULL}) — this method
     * doesn't need to know which; it just collects the string into an {@link MvpBallot}.
     * {@link MvpPersister} is what resolves the run's actual config and decides which
