...
public class UploadRunService {
    ...
    // The Underworld is the only supported map that enforces the registered-character floor below.
    // Every other area - the Fissure of Woe (038-seed-fow.xml), Domain of Anguish
    // (051-seed-domain-of-anguish.xml), and the Eye of the North dungeons (055-seed-dungeons.xml) -
    // leans on the plugin's own real-player gate as its pug filter rather than a guild-registration
    // minimum, so an upload for any of them is accepted no matter how few of its party are
    // registered characters. An allow-list (not the old exempt-list) so a newly seeded map is
    // exempt by default; add its id here only if it should enforce the floor.
    // See specs/features/dungeons.md, fow-and-party-size.md §10.1.
     ...
     * applies it now — the Fissure of Woe, Domain of Anguish and the Eye of the North dungeons are
     * exempt (see REGISTRATION_FLOOR_ENFORCED_MAP_IDS). The admin retroactive-wipe cleanup still
     * applies the same bar per run regardless of map (see
    ...
    public UploadRunResponse processUpload(String rawMachineKey, Integer pluginVersion, UploadRunRequest request) {
        ...
        // are still allowed, just not a majority of the party. Only the Underworld enforces this —
        // see REGISTRATION_FLOOR_ENFORCED_MAP_IDS above.
        if (REGISTRATION_FLOOR_ENFORCED_MAP_IDS.contains(party.mapId())) {
