...
class RunHistoryIntegrationTest extends AbstractIntegrationTest {
    ...
    void searchFiltersByPersonCharacterAndRoleCombined() throws Exception {
        ...
        runParticipantRepository.save(new RunParticipant(myRun, myChar, "MyToon", warrior, null, "T1", 0, true, false, false, 0));

        Run otherRun = seedRun(map, Instant.now(), true);
        runParticipantRepository.save(new RunParticipant(otherRun, null, "SomeoneElse", warrior, null, "spiker", 0, true, false, false, 0));
    ...
    void detailReturnsFullNestedRunWithObjectivesAndParticipantsInOrder() throws Exception {
        ...
        runParticipantRepository.save(new RunParticipant(run, null, "SlotTwo", ranger, null, "T2", 1, true, false, false, 0));
        runParticipantRepository.save(new RunParticipant(run, null, "SlotOne", warrior, null, "T1", 0, true, false, false, 0));
