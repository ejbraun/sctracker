...
- `default_role`, if provided, must be one of the 8 known role codes (`T1`/`T2`/`T3`/`T4`/`LT`/`spiker`/`sos`/`emo`) → `400` otherwise. It's advisory metadata only, never consulted by ingestion's role derivation (spec 02).
