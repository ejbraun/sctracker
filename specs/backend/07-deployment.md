...
- `plugins/<Name>/<Name>.dll` + `plugins/<Name>/<Name>.version.json` — one prefix per Toolbox plugin, e.g. `plugins/SCTracker/…`, `plugins/PP-Vanquish/…`. The toolbox build itself rides this layout too: `plugins/GWToolboxdll/GWToolboxdll.dll` (+ manifest), served as a public plugin under key `gwtoolbox`. It's registered with `ui_visible = false` (GWRL injects it; a user never places it by hand), so it's hidden from the website entirely — `GET /api/account/modules` and the `/plugins` page — while the machine-key path still serves it. Its manifest has no integer `version`, so the backend reports `version 0` for it.
...
- Optional third sidecar in either layout: `<Name>.patch.txt` — plain text, one per artifact, registered as `modules.patch_notes_object`. The publishing pipeline is expected to *append* to this file release over release and overwrite the bucket object with the accumulated text each publish; the backend has no append/versioning logic of its own, it only ever serves whatever's currently at that path (`GET /modules/{key}/patch-notes`, `GET /api/account/modules/{key}/patch-notes`) or 404s if the module has none configured.

`ModuleManifestCache` reads each `*.version.json` (`plugin.storage.module-cache-ttl`, default 15m) and streams the artifact bytes per request at `GET /modules/{key}/download` — bytes are never held in memory. Patch notes text is never cached either — read fresh from the bucket per request, same as the artifact. Optional overrides: `plugin.storage.module-cache-ttl`, `plugin.storage.max-module-download-bytes` (default 64 MiB).
